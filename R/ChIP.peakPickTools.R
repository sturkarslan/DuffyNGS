...
`findPeaks` <- function( tbl, peak.type=c("auto","gaussian","gumbel"), 
	...
	if ( visualize && names(dev.cur()) != "X11") X11( width=10, height=7, bg="white")
	...
	quantCut <- quantile( ctrlScore, 0.95)
...
`cleanTailRegion` <- function( x, y, centerX, width, flank=c("left","right")) {
	...
	medY <- quantile( y[nonpeakX], 0.25) * 1.15
	whoLow <- which( y <= medY)
