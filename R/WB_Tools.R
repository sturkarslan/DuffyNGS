...
`WB_setBinSizeBySpecies` <- function(x) { 
	...
	mysize <- 100
	if ( x == "Hs_grc") mysize <- 250
	if ( x == "Mmu_grc") mysize <- 250
	if ( x == "MacMu") mysize <- 250
	if ( x == "Tb_maxi") mysize <- 25
...
`WB_setSpeciesDetectability` <- function( ) {
	...
	if ( length( otherSpecies) > 0) otherID <- otherSpecies[1]

	# allow the default 'other species' to be human...
	if ( is.null(otherID) && (selfID != "Hs_grc")) otherID <- "Hs_grc"
