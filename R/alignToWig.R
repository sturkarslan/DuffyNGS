...
`alignToWig` <- function( filein, sampleID, reload=TRUE, optionsFile="Options.txt", readSense="sense", 
			dataType="RNA-seq", results.path=NULL, maxReads=NULL, readBufferSize=1000000, 
			verbose=TRUE) {
	...
	`scratchFileName` <- function( sampleID, seq, strand) {
		return( file.path( tempWigPath, base::paste( sampleID, seq, strand, "tmp.rda", sep=".")))
	}
	...
	`eraseAllOutputWigFiles` <- function( sampleID) {
		for ( s in mySpeciesSet) {
			...
			thisfile <- file.path( wigPath, base::paste( sampleID, prefix, "WIG.rda", sep="."))
			if ( file.exists( thisfile)) file.remove( thisfile)
			thisfile <- file.path( wigPath, base::paste( sampleID, prefix, "Plus.wig", sep="."))
			if ( file.exists( thisfile)) file.remove( thisfile)
			thisfile <- file.path( wigPath, base::paste( sampleID, prefix, "Minus.wig", sep="."))
			if ( file.exists( thisfile)) file.remove( thisfile)
			thisfile <- file.path( wigPath, base::paste( sampleID, prefix, "Plus.unique.wig", sep="."))
			if ( file.exists( thisfile)) file.remove( thisfile)
			thisfile <- file.path( wigPath, base::paste( sampleID, prefix, "Minus.unique.wig", sep="."))
			if ( file.exists( thisfile)) file.remove( thisfile)
			...
			seqSet <- getCurrentSeqMap()$SEQ_ID
			for (seq in seqSet) {
				thisfile <- file.path( subwigPath, base::paste( sampleID, seq, "WIG.rda", sep="."))
				if ( file.exists( thisfile)) file.remove( thisfile)
			}
		}
	}
	...
	`compressWIGfromScratchFiles` <- function( sampleID) {
		...
		storedReadLength <- RawReadLength
		...
		for ( s in mySpeciesSet) {
			...
			for (iseq in 1:length(seqSet)) {
				...
				if ( ! file.exists( thisfile)) {
					...
					infoP <- saveNaligns
					storedReadLength <- max( storedReadLength, saveReadLen)
				...
				if ( storageMode == "combined") {
					...
					thisFile <- file.path( subwigPath, base::paste( sampleID, seq, "WIG.rda", 
								sep="."))
			...
			wigFile <- base::paste( sampleID, prefix, "WIG.rda", sep=".")
		...
		for ( s in mySpeciesSet) {
			...
			expandOneWiggleChuckToScratchFile <- function( iseq) {
				...
				if ( nrow( saveDF) < 1) {
					...
					save( saveDF, saveNaligns, saveReadLen, saveFileSet, savedAsTable, file=thisfile)
				}
	...
	`OneSeqOneStrand2wiggles` <- function( x) {
		...
		saveReadLen <- max( saveReadLen, RawReadLength)
		saveFileSet <- FileNameSet
		save( saveDF, saveNaligns, saveReadLen, saveFileSet, savedAsTable, file=saveFile)
		...
		savedAsTable <- FALSE
		thisBaseVec <- baseDepthVector( startSet, stopSet, wtSet)
		saveFile <- scratchFileName( sampleID, thisSeqID, base::paste( thisStrand, "unique", sep="."))
		#if ( ! reload) {
	...
	`writeWIGfiles` <- function( sampleID, species) {
		...
		for( iseq in 1:length( seqSet)) {
			...
			seqIDout <- sub( base::paste( "^", species, "_", sep=""), "", thisSeq)
			seqIDout <- sub( "^0", "", seqIDout)
			if ( base::substr( seqIDout, 1,3) != "chr") seqIDout <- base::paste( "chr", seqIDout, sep="")
			...
			sml <- base::paste( seqOut, as.integer(startsOut), as.integer(endsOut), dataOut, sep="\t")
	...
	if ( reload) {
		...
		FileNameSet <- base::append( curfiles, filein)
	}
	...
	repeat {
		...
		if ( nbuf == 1) RawReadLength <- round( mean( readLens))
		...
		#totalWiggleCountPerSpecies <- tallyWiggleCountsBySpecies(cnts)
	...
	# now write the text format for genome viewing...
	#for ( s in mySpeciesSet) writeWIGfiles( sampleID, s)
	cat( "\n")
