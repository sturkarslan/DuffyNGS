...
function( inputFastqFile, outputFile=sub( "fastq$", "bam", inputFastqFile[1]),  
		...
		maxReads=NULL, skipReads=NULL, verbose=TRUE, debug=FALSE) {
	...
	trim5 <- as.integer( getOptionValue( optT, "trim5", notfound="0"))
	trim3 <- as.integer( getOptionValue( optT, "trim3", notfound="0"))
	...
	# the index
	myIndexFile <- file.path( getOptionValue( optT, "bowtie2Index.path", notfound=".", verbose=verbose), alignIndex)
