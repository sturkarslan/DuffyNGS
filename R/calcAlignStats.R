...
mergeIndelStats <- function( stats1, stats2) {
	...
	newPos[ 1:len1] <- newPos[ 1:len1] + stats1$InsertPositions
	newPos[ 1:len2] <- newPos[ 1:len2] + stats2$InsertPositions
	newInsertPos <- newPos
	len1 <- length( stats1$DeletePositions)
	len2 <- length( stats2$DeletePositions)
	lenNew <- max( c( len1, len2))
	newPos <- rep( 0, times=lenNew)
	newPos[ 1:len1] <- newPos[ 1:len1] + stats1$DeletePositions
	newPos[ 1:len2] <- newPos[ 1:len2] + stats2$DeletePositions
	newDeletePos <- newPos
...
plotIndelStatData <- function( alignStatsData, asPNG=FALSE, plotPath=".", pause=0) {
	...
	names(insPatt) <- paste( "Ins:", names(insPatt), sep=" ")
	names(delPatt) <- paste( "Del:", names(delPatt), sep=" ")
