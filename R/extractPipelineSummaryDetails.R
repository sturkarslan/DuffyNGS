...
`extractPipelineSummaryDetails` <- function( sampleID, optionsFile="Options.txt", 
	...
	target <- getOptionValue( optionsFile, "targetID", notfound="HsPf", verbose=FALSE)
	...
	out <- data.frame( "SampleID"=sampleID, "RawReads"=nRaw, "N_NoHit"=nNoHit, "P_NoHit"=pNoHit,
			 "N_RiboClear"=nRibo, "P_RiboClear"=pRibo,
			 "N_Genomic"=nGenomic, "P_Genomic"=pGenomic,
			 "N_Splice"=nSplice, "P_Splice"=pSplice, stringsAsFactors=FALSE)
