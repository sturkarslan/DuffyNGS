...
`pipe.DESeq` <- function( sampleIDset, speciesID, annotationFile="Annotation.txt",
				...
				doDE=TRUE, PLOT.FUN=NULL, ...)
{
	...
	minimumRPKM <- as.numeric( getOptionValue( optT, "DE.minimumRPKM", notfound="1"))
	...
	if (doDE) {
		...
		out <- DESeq.DiffExpress( transFileSet, transFIDs, groupSet=RP_groups, targetGroup=targetgroup, 
				...
				minimumRPKM=minimumRPKM, keepIntergenics=keepIntergenics, 
		...
		if ( RP_prefix %in% c("Pf", "Pb", "Pv", "Py", "Py17X","Pym", "PvSal1")) {
