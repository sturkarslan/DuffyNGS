...
`pipe.EdgeR` <- function( sampleIDset, speciesID, annotationFile="Annotation.txt",
				...
				doDE=TRUE, PLOT.FUN=NULL, ...)
{
	...
	if (doDE) {
		...
		if ( nrow(out1) > 0) {
			...
			colnames(out1)[c(3:5 + extraCols)] <- c( "Log2 Fold", "P Value", "FDR")
			# write it
		...
		if ( nrow(out2) > 0) {
			...
			colnames(out2)[c(3:5 + extraCols)] <- c( "Log2 Fold", "P Value", "FDR")
			# write it
