...
`pipe.QuickQC` <- function( sampleID, annotationFile="Annotation.txt",
		...
		chunkSize=250000, maxReads=5000000, pause=0, verbose=T, nUSRkeep=1000000,
	...
	targetID <- getOptionValue( optTbl, "targetID", notfound="HsPf")
	...
	# set up sub folders
	folderSet <- c( "align", "riboClear", "fastq", "splicing", "USR", "CR")
	if ( dataType == "ChIP-seq") folderSet <- c( "align", "ChIPpeaks", "fastq", "USR", "CR")
	...
	if ( dataType == "RNA-seq" && spliceIndex != "") {
	...
	if ( dev.cur() < 2) X11( width=8, height=6, xpos=10, ypos=10, bg="white")
