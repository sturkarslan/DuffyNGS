...
function( inputFastqFile, sampleID, annotationFile="Annotation.txt", optionsFile="Options.txt",
		verbose=TRUE, rawReadCount=NULL)
{
	...
	finalNohit <- paste( sampleID, "not.ribo.fastq.gz", sep=".")
	...
	ans <- fastqToBAM( inputFastqFile, finalHit, k=maxMultiHits, sampleID=sampleID,
			noHitsFile=finalNohit, optionsFile=optionsFile, 
			alignIndex=alignIndex, alignPolicy=alignPolicy, verbose=verbose)
