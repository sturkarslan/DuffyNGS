...
`pipe.SpliceCoverageAnalysis` <- function( sampleIDset, annotationFile="Annotation.txt",
				...
				groupColumn="Group", tool=c("SAM","RankProduct","EdgeR")) {
	...
	tool <- match.arg( tool)
	intensityColumn <- if (tool == "EdgeR") "N_READS" else "PCT_READS"
	...
	if ( NTARGETS > 1) {
		for ( k in 1:NTARGETS) {
			...
			if (tool == "SAM") {
				...
				ansDE[[k]] <- EdgeR.DiffExpress( files, fids, groupSet=groups, targetGroup=targets[k],
						geneColumn="KEY_ID", intensityColumn=intensityColumn,
						minimumIntensity=2)
			}
			saveAns <<- ansDE
