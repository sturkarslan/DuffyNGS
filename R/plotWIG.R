...
`plotWIGgene` <- function( WIG, gene, tailWidth=1000, plotType=c("boxes", "lines", "segments", "addedLines", "none"), 
			...
			forceYmax=NULL, minYmax=10, showDetectability=TRUE, legend.cex=1) {
...
`annotateWIGplot` <- function( seqID,  gene=NULL, xlim, annoSize, speciesID, offset=0, 
	...
	if ( !fastMode && nrow( gmap) > 0) {
	   for( i in 1:nrow( gmap)) {
		...
		xright <- gmap$END[i]
		if ( regexpr( '@', gmap$GENE_ID[i], fixed=T) > 0) next
		...
		if ( gmap$REAL_G[i] == TRUE) rect( xleft, exonYlo, xright, exonYhi, border=1, lwd=1)
	...
	if ( nrow( emap) > 0) {
	   ...
	   for( i in 1:nrow( emap)) {
		xleft <- emap$POSITION[i]
		xright <- emap$END[i]
		strand <- if ( is.na( emap$STRAND[i])) " " else emap$STRAND[i]
		if ( strand %in% c( "+", " ")) {
			exonYlo <- exonYloPlus;  exonYhi <- exonYhiPlus;
		} else {
			exonYlo <- exonYloMinus;  exonYhi <- exonYhiMinus;
	...
	# var gene domains
	if ( exists( "vargeneDomainMap") && getCurrentSpecies() == "Pf3D7") {
		varmap <- subset.data.frame( vargeneDomainMap, GENE_NAME %in% gmap$GENE_ID)
		if ( nrow( varmap) > 0) {
			strands <- ifelse( is.na( varmap$STRAND), " ", varmap$STRAND)
			vgname <- ""
			for ( i in 1:nrow( varmap)) {
				xleft <- varmap$POSITION[i]
				xright <- varmap$END[i]
				strand <- strands[i]
				if ( varmap$GENE_NAME[i] != vgname) {
					if ( strand %in% c( "+", " ")) {
					...
					toggle <- 1
					vgname <- varmap$GENE_NAME[i]
				}
				ytoggle <- varYtoggleSize * toggle
				lines( x=c(xleft,xright), y=rep( varYcenter + ytoggle, 2), lty=1, lwd=6, 
					col="goldenrod")
				textX <- (xleft+xright)/2
				text( x=textX, y=varYcenter - ytoggle, labels=varmap$DOMAIN_ID[i], pos=NULL, 
					cex=mycex*0.9, font=2)
				toggle <- ( - toggle)
			}
		}
	} 
	...
	if ( showDetectability && !is.null( curDetectSelf) && !is.na( curDetectSelf$Unique)) {
	    ...
	    if ( length(toDo) > 0) {
	    if ( asbox) {
		rect( xl[toDo], yl[toDo], xr[toDo], yh[toDo], border=3, density=12, col=3)
	    } else {
	    ...
	    if ( hasOther) {
	    	...
	    	if ( length(toDo) > 0) {
	    	if ( asbox) {
			rect( xl[toDo], yl[toDo], xr[toDo], yh[toDo], border=2, density=12, col=2)
	    	} else {
	    		# make segments with NA's
			Ntodo <- length(toDo)  *3
			xx <- rep( NA, times=Ntodo)
...
`makeAllWIGgenePlots` <- function( WIG, geneSet=NULL, path="png", fileExtension="png", tailWidth=2000, 
	...
	for ( i in 1:length( geneSet)) {
		...
		if ( geneNameColumn != "GENE_ID") {
			gplotname <- geneMap[[geneNameColumn]][ match(g,geneMap$GENE_ID)]
		}
