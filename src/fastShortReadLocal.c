...
void fastShortReadLocal( int *id1, int *n1, int *id2, int *n2, double *misMatchPenalty, double *gapPenalty, double *score, int *offset, int *gaps) {
	...
	for(i = 1; i < size1; i++) {
		...
		for(j = 1; j < size2; j++) {
			...
			// local align:  never let score go negative
			if ( F[ij] < 0) {
				F[ij] = 0.0;
				T[ij] = 0;
			}
	...
	// local align:  the best score can be anywhere...
	bestF = MISMATCH;
	best1= 0;
	best2= 0;
	...
	// local align: we bactrack only til a zero score
	start1 = best1;
	start2 = best2;
	gapLoc1 = 0;
	gapLen1 = 0;
