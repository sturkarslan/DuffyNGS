...
bam_header_t *bam_header_read(bamFile fp)
{
	...
	if (i < 0) {
		...
		Rprintf("Info:  BAM file has no EOF marker.\n");
	// read "BAM1"
...
char *bam_aux_format1( const uint8_t *s)
{
	int i;
	kstring_t str;
	str.l = str.m = 0; str.s = 0;
	...
	uint8_t type, key[2];
	type = *s; ++s;
	if (type == 'A') { kputc(*s, &str); ++s; }
	else if (type == 'C') { kputw(*s, &str); ++s; }
	else if (type == 'c') { kputw(*(int8_t*)s, &str); ++s; }
	else if (type == 'S') { kputw(*(uint16_t*)s, &str); s += 2; }
	else if (type == 's') { kputw(*(int16_t*)s, &str); s += 2; }
	else if (type == 'I') { kputuw(*(uint32_t*)s, &str); s += 4; }
	else if (type == 'i') { kputw(*(int32_t*)s, &str); s += 4; }
	else if (type == 'f') { ksprintf(&str, "%g", *(float*)s); s += 4; }
	else if (type == 'd') { ksprintf(&str, "%lg", *(double*)s); s += 8; }
	else if (type == 'Z' || type == 'H') { while (*s) kputc(*s++, &str); ++s; }
	else if (type == 'B') {
		uint8_t sub_type = *(s++);
		int32_t n;
		memcpy(&n, s, 4);
...
char *bam_aux_formatAll(const bam1_t *b, const char *sep)
{
	uint8_t *s;
	int i;
	...
	while (s < b->data + b->data_len) {
		...
		key[0] = s[0]; key[1] = s[1];
		s += 2; type = *s; ++s;
		kputsn((char*)key, 2, &str); kputc(':', &str);
		if (type == 'A') { kputsn("A:", 2, &str); kputc(*s, &str); ++s; }
		else if (type == 'C') { kputsn("i:", 2, &str); kputw(*s, &str); ++s; }
		else if (type == 'c') { kputsn("i:", 2, &str); kputw(*(int8_t*)s, &str); ++s; }
		else if (type == 'S') { kputsn("i:", 2, &str); kputw(*(uint16_t*)s, &str); s += 2; }
		...
		// add the sep if there's more
		if (s < b->data + b->data_len) kputsn( sep, seplen, &str);
	}
